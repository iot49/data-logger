...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MyHomePage(title: 'UART example'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _sendData() async {
    var data = _dataToSendText!.text;
    _logTexts = "${_logTexts}_sendData:$data \n";
    //final mtu = await flutterReactiveBle.requestMtu(deviceId: _deviceid, mtu: 250);
    //_logTexts = "${_logTexts}MTU = $mtu \n";
  ...
  void onNewReceivedData(List<int> data) {
    ...
    var len = data.length;
    _logTexts = "${_logTexts}RECV [$len] $data \n";
    _receivedData
  ...
  void _disconnect() async {
    // TODO: detect when remote disconnected
    ...
    if (goForIt) {
      ...
      _scanStream = flutterReactiveBle
        ...
        if (_foundBleUARTDevices.every((element) => element.id != device.id)) {
          ...
          _logTexts = "${_logTexts}INFO found:$name \n";
          _logTexts = "${_logTexts}_$device \n\n";
